   ...
   */
   ...
   while (glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS && !glfwWindowShouldClose(window))
   {
      ...
      for( int y = 0 ; y < WINDOW_HEIGHT; y++){
         for( int x = 0 ; x < WINDOW_WIDTH; x++ , screen += 4){

            float aa = tan_fovx * ( ( x - WINDOW_WIDTH/2.0) / (float)( WINDOW_WIDTH / 2.0 ) );
            float bb = tan_fovy * ( (y - (WINDOW_HEIGHT/2.0)) / (float)( WINDOW_HEIGHT/ 2.0) );

            glm::vec4 right = glm::vec4(camera.getRightVector(), 0.0f);
            glm::vec4 up    = glm::vec4(camera.getUpVector(), 0.0f);
            glm::vec4 view  = glm::vec4(camera.getViewingDirection(), 0.0f);

            //glm::vec4 rayDirection = (aa * glm::vec4(1.0, 0.0, 0.0, 0.0) ) + ( bb * glm::vec4(0.0 , 1.0, 0.0, 0.0) ) + glm::vec4( 0.0, 0.0, -1.0, 0.0);
            glm::vec4 rayDirection = (aa * right ) + ( bb * up ) + view;
            glm::vec4 rayOrigin = glm::vec4(camera.getPosition(), 1.0f);

            rayDirection = glm::normalize(rayDirection);

            // transform ray
            rayOrigin = inv_M * rayOrigin;
            rayDirection = inv_M * rayDirection;

            glm::vec3 rOrigin(rayOrigin.x, rayOrigin.y, rayOrigin.z);
            glm::vec3 rDirection( rayDirection.x, rayDirection.y, rayDirection.z);

            Ray ray(rOrigin, rDirection);
            float dummy;
            bool intersectionFound = mySphere.hit(ray, rayIntersection, dummy);
          
            if(intersectionFound){

               glm::vec4 intersection = glm::vec4( rayIntersection.getPoint(), 1.0);
               glm::vec4 intersectionNormal = glm::vec4( rayIntersection.getNormal(), 0.0);
               
               // change intersection point and normal from object to World space
               intersection = M * intersection;
               intersectionNormal = glm::transpose(inv_M) * intersectionNormal;

               glm::vec4 intersectionToLight = lightPosition - intersection;
               intersectionToLight =  glm::normalize(intersectionToLight);

               float theta = glm::dot( intersectionNormal, intersectionToLight);
               
               glm::vec4 finalColor(0.0f); 
               finalColor += sphereColor * lightColor * lightAmbientIntesity;
               if( theta >= 0.0 ){
                                      
                  finalColor += theta * sphereColor * lightColor;
                  
                  glm::vec4 V = glm::vec4(rDirection,0.0f);
                  glm::vec4 R = intersectionToLight - 2.0f *glm::dot(intersectionToLight, intersectionNormal) * intersectionNormal;
                  float dotSpecular = glm::dot(V, R);
                  if (dotSpecular > 0)
                  {
                     //float spec = powf( dotSpecular, 20 ) * sphereColor * glm::vec3(1.0f,1.0f,);
                     // add specular component to ray color
                     float spec = powf(dotSpecular,155);
                     finalColor +=  spec * glm::vec4(1.0f, 1.0f, 1.0f,1.0f) * lightColor ;
                  }

                  //screen[0] = floor(finalColor.x == 1.0 ? 255 : finalColor.x * 256.0);
                  //screen[1] = floor(finalColor.y == 1.0 ? 255 : finalColor.y * 256.0);
                  //screen[2] = floor(finalColor.z == 1.0 ? 255 : finalColor.z * 256.0);
                  //screen[3] = 0;
               }
               imageBuffer[ 4 * (x + y * WINDOW_WIDTH)] = floor(finalColor.x == 1.0 ? 255 : std::min(finalColor.x * 256.0, 255.0));
               imageBuffer[1 +  4 * (x + y * WINDOW_WIDTH)] = floor(finalColor.y == 1.0 ? 255 : std::min(finalColor.y * 256.0, 255.0));
               imageBuffer[2 +  4* (x + y * WINDOW_WIDTH)] = floor(finalColor.z == 1.0 ? 255 : std::min(finalColor.z * 256.0, 255.0));
               imageBuffer[3 +  4* (x + y * WINDOW_WIDTH)] = 0;
            }         
         }
      }
      */
